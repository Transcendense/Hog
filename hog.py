...
def swine_align(player_score, opponent_score):
    ...
    "*** YOUR CODE HERE ***"
    res=1
    for i in range(1,min(player_score,opponent_score)+1):
        if (not player_score%i) and (not opponent_score%i):
            res=i
    if res>=10 and opponent_score>=10:
        return True
    else:
        return False
    # END PROBLEM 4a
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if who:
            ...
            tempscore = take_turn(num_dice,score0,dice)
            score1 += tempscore
        else:
            num_dice = strategy0(score0, score1)
            tempscore = take_turn(num_dice,score1,dice)
            score0 += tempscore
        if who:
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def commentary(score0, score1, last_score=last_score, running_high=running_high):
        ...
        score_increase = current_score - last_score

        if score_increase > running_high:
            running_high = score_increase
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    for num_dice in range(1, 11):
        temp_func = make_averaged(roll_dice, trials_count)
        temp = temp_func(num_dice, dice)
        if temp > rec:
            res = num_dice
            rec = temp
    return res
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    return bacon_strategy(score, opponent_score, cutoff, num_rolls) * (not extra_turn(score+free_bacon(opponent_score), opponent_score))
